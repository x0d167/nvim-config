...
vim.api.nvim_create_autocmd("FileType", {
	callback = function()
		...
		-- Get the current file's type
		local ft = vim.bo.filetype

		-- If it's C or C++, let Neovim use native cindent instead of Treesitter indent
		if ft == "c" or ft == "cpp" then
			vim.opt_local.cindent = true
			vim.opt_local.smartindent = true
			vim.opt_local.indentexpr = "" -- Empty string tells Neovim to look at cindent/smartindent
		else
			-- Keep using treesitter indent for everything else (Python, JS, Go, etc.)
			vim.opt_local.indentexpr = "v:lua.require'nvim-treesitter'.indentexpr()"
		end
	end,
})
